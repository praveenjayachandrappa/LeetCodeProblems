...
public class CombinationSum {
    ...
    private void generateCombination(int[] cand,int start,int target,List<Integer> temp){
        if(target<0)
            return;
        if(target==0){
        ...
        for(int i=start;i<cand.length;i++){
            temp.add(cand[i]);
            generateCombination(cand,i,target-cand[i],temp);
            temp.remove(new Integer(cand[i]));
        }
    }
